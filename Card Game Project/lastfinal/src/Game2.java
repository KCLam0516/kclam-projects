...
public class Game2 extends Game {
	...
	public void startgame1(List<Playerprof>Players)
	{
		do {
...
//  				i = input.nextInt();   //user input to select which card
			...
			Card.Compare4(selcard.get(0).getValue(),selcard.get(1).getValue(),selcard.get(2).getValue(),0);
			for(int k=1;k<=Numplayer;k++)
			{
				...
				for(int p = 0; p <Numplayer; p++)
				{
					...
					{
						System.out.println("Sorry, Player " + player.getName() + " is not enough card to play game!"); //tell player that he is not enough card to play war
						unusedcard.addAll(PlayerDeck[p]); //add all the player card that already lose to unused card
						PlayerDeck[p].clear();
						//clear the player deck because already lose
						//Players.remove(p); //remove player profile
					}
				}
		int gg=0;
		if(Card.GetResult()==0&& PlayerDeck[0].size() >= 4 && PlayerDeck[1].size() >= 4 &&PlayerDeck[2].size() >= 4)
				firstwar1();
		else if(Card.GetResult()==0&&PlayerDeck[0].size() < 4 && PlayerDeck[1].size() < 4 &&PlayerDeck[2].size() < 4)
		...
		}while(PlayerDeck[0].size() >= 1 && PlayerDeck[1].size() >= 1 && PlayerDeck[2].size() >= 1);//check whether enough card or not to continue game
		displayfinalresult();
	}
...
//				int s = input.nextInt(); //let user to select 4 card
		...
		Card.Compare4(war[0].get(3).getValue(),war[1].get(3).getValue(),war[2].get(3).getValue(),0);
		for(int k=1;k<=Numplayer;k++)
		{
			Playerprof player = getNextPlayer();
			if(Card.GetResult()==k)
		...
		if(PlayerDeck[0].size() < 1 && PlayerDeck[1].size() < 1 && PlayerDeck[2].size() < 1)
		{
			for(int p = 0; p <Numplayer; p++)
			{
				Playerprof player = getNextPlayer();  //get player profile
...
//			int s = input.nextInt();      //user input to select which card to fight with other players
		...
		Card.Compare4(war[0].get(1).getValue(),war[1].get(1).getValue(),war[2].get(1).getValue(),0);
		for(int k=1;k<=Numplayer;k++)
		{
			Playerprof player = getNextPlayer();
			if(Card.GetResult()==k)
		...
		for(int p = 0; p <Numplayer; p++)
		{
			...
			if(PlayerDeck[p].size() <= 1)
			{
				...
				unusedcard.addAll(PlayerDeck[p]); // add the player deck into unused card
				PlayerDeck[p].clear();// clear the player deck that lose
				Players.remove(p); //remove the player profile
			}
		}
